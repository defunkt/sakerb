module FormTestHelper
  ...
  class Form
    ...
    def submit(opts={})
      raise MissingSubmitError, "Submit button not found in form" unless tag.search('input[@type="submit"], input[@type="image"], button[@type="submit"]').any?
      @xhr = opts.delete(:xhr)
    ...
    def fields
      ...
      @fields ||= tag.search('input, textarea, select, button').reject {|field_tag| field_tag['name'].nil? }.group_by {|field_tag| field_tag['name'] }.collect do |name, field_tags|
  ...
  class Select < Field
    def initialize(tags)
      @options = tags.first.search("option").collect {|option_tag| Option.new(self, option_tag) }
      super
    end
  ...
  def select_form(text=nil, use_xhr=false)
    ...
    when text.nil?
      doc.search('form')
    when doc.search(%Q{form[@action="#{text}"]}).any?
      doc.search("form[@action=#{text}]")
    else
